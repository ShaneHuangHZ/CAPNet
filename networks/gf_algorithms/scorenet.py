...
from utils.capnet_utils import get_pose_dim
        ...
        if self.regression_head == 'RT':
            ...
            if per_point_feature:
                ...
                self.offset = nn.Sequential(
                    ...
                    self.act,
                    zero_module(nn.Conv1d(64,3,1)),
                )
                self.nocs_layer = nn.Sequential(
                    nn.Conv1d(64, 64, 1),
                    # nn.Linear(64, 64),
                    nn.BatchNorm1d(64, eps=1e-05, momentum=0.1, affine=True, track_running_stats=True), 
                    self.act,
                    zero_module(nn.Conv1d(64, 10*3*cfg_bins['ce_loss_bins'],1)),
                )
    ...
    def forward(self, data):
        ...
        if self.per_point_feature:
            ...
            if self.dino_dim:
                ...
                total_feat = torch.cat([pts_feat], dim=-1)
        ...
        if self.regression_head == 'RT':
            ...
            offset = self.offset(total_feat) #total_feat.shape torch.Size([16, 64, 1024])
            nocs = self.nocs_layer(total_feat)
            sem = self.rgbd_seg_layer(total_feat)
